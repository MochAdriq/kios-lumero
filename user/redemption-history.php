  ...
  <section class="reveal reveal-delay-1">
    ...
    <div class="ticket-list">
      ...
      <article class="ticket">
        ...
        <div class="ticket-qr" onclick="openQrModal('<?=rh_e($qr)?>')">
          <img src="<?=rh_e($qr)?>" alt="QR kode penukaran">
          <span>Tap QR untuk perbesar</span>
        </div>
        <?php if (in_array($r['status'], ['requested','approved'])):
            $pName = $r['product_name'] ?? 'Hadiah';
            $rCode = $code;
            $waMsg = rawurlencode("Halo! Saya {$memberName} ingin mengambil hadiah: {$pName}. Kode kupon saya: {$rCode}. Mohon konfirmasi ya Kak! \xf0\x9f\x99\x8f");
            $waLink = $storeWaNum !== '' ? "https://wa.me/{$storeWaNum}?text={$waMsg}" : "https://wa.me/?text={$waMsg}";
        ?>
        <div style="padding:12px 16px 16px; border-top:1px dashed var(--border);">
            <a href="<?=rh_e($waLink)?>" target="_blank" rel="noopener"
               style="display:flex;align-items:center;justify-content:center;gap:8px;background:#25D366;color:#fff;padding:12px 20px;border-radius:99px;font-weight:800;font-size:14px;text-decoration:none;box-shadow:0 6px 16px rgba(37,211,102,0.3);">
            ...
            <p style="font-size:11px;color:var(--subtle);text-align:center;margin-top:8px;font-weight:600;">Tunjukkan pesan WA terkirim + QR di atas ke Kasir</p>
        </div>
        <?php endif; ?>
      </article>
      <?php endforeach; ?>
