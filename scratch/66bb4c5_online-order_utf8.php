...
<style>
/* ΓöÇΓöÇ Delivery Map Custom Styles ΓöÇΓöÇ */
.fo-search-results {
...
/* ============================================
   Lumero SELF-ORDER ΓÇô DARK PREMIUM CINEMATIC
   Adapted from POS kasir2-theme
   ============================================ */

/* ΓöÇΓöÇ Design Tokens (copas dari POS) ΓöÇΓöÇ */
/* ΓöÇΓöÇ Design Tokens (copas dari POS) ΓöÇΓöÇ */
:root{
  ...
  <header class="fo-header-bar w-100">
    <div class="fo-brand">
      ...
      <div>
        ...
        <small>Pesan Cepat ΓÇó Tanpa Antre ΓÇó Ambil di Outlet</small>
      </div>
    </div>
                            ...
                            <div class="sim-pos-topbar">
                                ...
                                <div class="sim-pos-topbar-right">
                                    ...
                                    <div class="input-icon-start search-pos position-relative">
                                        ...
                                        <input type="text" class="form-control" id="posSearch" placeholder="Cari produk... (ΓîÿK)">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="sim-order-meta">
                                    ...
                                    <div class="sim-order-type-toggle" style="display:none !important;">
                                        <select class="form-select form-select-sm" id="customerType" onchange="selectPickupOption(this.value)">
                                            <option value="outlet">≡ƒÅ¬ Ambil di Outlet</option>
                                            <option value="delivery" <?= $deliveryEnabled ? '' : 'disabled' ?>>≡ƒ¢╡ Delivery (Diantar Kurir) <?= $deliveryEnabled ? '' : '(Belum Aktif)' ?></option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
<div class="fo-floating-actions">
  <button type="button" class="fo-float-btn ai" onclick="toggleAiPanel()">Γ£¿ Dengarkan Saran AI</button>
  <a class="fo-float-btn wa" href="https://wa.me/6285794532040?text=Halo%20D%27Celup%20Pasekon%2C%20saya%20ingin%20bertanya%20tentang%20online%20order" target="_blank">≡ƒÆ¼ Hubungi Outlet</a>
</div>
...
<div class="fo-added-modal" id="addedModal" aria-modal="true" role="dialog">
  <div class="fo-added-card">
    <div class="fo-added-icon">Γ£ô</div>
...
<div class="fo-pickup-confirm" id="pickupConfirmModal" aria-modal="true" role="dialog">
  <div class="fo-pickup-confirm-card">
    ...
    <div class="fo-checkout-grid" style="margin-top:12px" id="outletDateTimeGrid">
      ...
      <div class="fo-field"><label>Jam Pengambilan</label><select id="pickupTime"></select><div class="fo-time-hint" id="pickupTimeHint">Menyiapkan opsi waktu pengambilanΓÇª</div></div>
    </div>
    ...
    <div id="deliverySectionWrap" style="display:none; margin-top:14px;">
      <div style="margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; background:var(--dp-surface); border:1px solid var(--dp-glass-border); padding:10px 12px; border-radius:10px;">
        <div style="font-size:12px; font-weight:600; color:var(--dp-text); display:flex; align-items:center; gap:6px;">
          <span>≡ƒôì</span> <span>Lokasi Pengantaran</span>
        </div>
        <button type="button" id="btnUseGps" onclick="useCurrentDeviceLocation()" style="background:var(--dp-gradient); color:#fff; border:none; padding:7px 14px; border-radius:8px; font-size:12px; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:6px; box-shadow:0 2px 10px rgba(255,45,85,0.3); transition:all 0.2s;">
          ≡ƒÄ» Gunakan Lokasi Saat Ini (GPS)
        </button>
      </div>
      ...
      <div style="margin: 8px 0; border-radius:10px; overflow:hidden; border:1px solid var(--dp-glass-border); position:relative;">
        ...
        <div style="position:absolute; bottom:8px; left:8px; right:8px; z-index:400; background:rgba(15,15,19,0.85); backdrop-filter:blur(10px); padding:6px 10px; border-radius:8px; border:1px solid var(--dp-glass-border); font-size:11px; display:flex; justify-content:space-between; align-items:center;">
          <span style="color:#fff;">≡ƒôì Geser pin ke titik lokasi pasti</span>
...
<div class="fo-drawer" id="checkoutDrawer"><div class="fo-panel">
  ...
  <div class="fo-pay-preview active" id="qrisPreview" style="text-align:center; padding:20px;">
    ...
    <div style="margin-top:16px;">
      <a class="fo-download-btn" href="../<?=fo_e(ltrim($paymentQrisImage,'/'))?>" download="QRIS-Lumero.png" style="box-shadow:0 4px 16px rgba(255,45,85,0.4); padding:10px 24px; font-size:13px;">Γ¼ç∩╕Å Download QRIS</a>
    </div>
...
/* ΓöÇΓöÇ Delivery JS Configuration & State ΓöÇΓöÇ */
...
function initDeliveryMap() {
  ...
  const outletIcon = L.divIcon({
    ...
    html: '<div style="background:#ff2d55; border:2px solid #fff; border-radius:50%; width:24px; height:24px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:12px; font-weight:bold; box-shadow:0 2px 6px rgba(0,0,0,0.5);">≡ƒÅá</div>',
    iconSize: [24, 24],
    iconAnchor: [12, 12]
  });
  ...
  const pinIcon = L.divIcon({
    ...
    html: '<div style="background:#34d399; border:2px solid #fff; border-radius:50%; width:28px; height:28px; display:flex; align-items:center; justify-content:center; color:#1a1a2e; font-size:14px; font-weight:bold; box-shadow:0 2px 8px rgba(0,0,0,0.6); cursor:move;">≡ƒôì</div>',
    iconSize: [28, 28],
    iconAnchor: [14, 14]
  });
...
function useCurrentDeviceLocation() {
  ...
  const origText = btn ? btn.innerHTML : '≡ƒÄ» Gunakan Lokasi Saat Ini (GPS)';
  if (btn) btn.innerHTML = 'ΓÅ│ Mengambil Titik GPS...';
...
function buildPickupOptions(){
  ...
  if(autoDate!==today){
    ...
    if(serverNowTime >= '21:00' && selectedDate===today){
      ...
      placeholder.value=''; placeholder.textContent='Outlet sudah tutup ΓÇó pilih besok';
...
function renderCart(){
  ...
  if(!cart.length){
    ...
    if(sideList){
      sideList.innerHTML='<div class="fo-order-empty"><div class="icon">≡ƒ¢Æ</div><p>Belum ada menu dipilih</p><small>Klik menu di kiri untuk menambah pesanan</small></div>';
    }
  }
...
function updateFooterDetail(total,count){
  ...
  el.textContent=`${count} item ΓÇó ${names.join(', ')}${more} ΓÇó Pilih Lanjut Bayar untuk checkout.`;
}
...
function updateAudioToggleButtons(){
  ...
  if(musicBtn){
    ...
    musicBtn.innerHTML=isMusicOn()?'<span>ΓÖ¬</span> Musik ON':'<span>ΓÖ¬</span> Musik OFF';
  }
  if(guideBtn){
    guideBtn.classList.toggle('active',isGuideOn());
    guideBtn.classList.toggle('off',!isGuideOn());
    guideBtn.innerHTML=isGuideOn()?'<span>≡ƒöè</span> Suara ON':'<span>≡ƒöç</span> Suara OFF';
  }
}
