...
foreach ($newMaterials as $name) {
    ...
    if (!$existing) {
        ...
        $stmt = $pdo->prepare("INSERT INTO raw_materials (name, sku, category_id, unit_id, stock_qty, min_stock_qty, average_cost, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 1, 0, NOW(), NOW())");
        $stmt->execute([$name, $sku, $categoryId, $unitId]);
        echo "Berhasil menambahkan bahan baku: $name (Unit ID: $unitId, Cat ID: $categoryId)\n";
    } else {
